...
def pancard_main(image_path):
    ...
    simple_image_string = pytesseract.image_to_string(image)
    ...
    english_string = pytesseract.image_to_string(image ,config=custom_config)


    
    pancard_number = extract_pan_number(simple_text_blank_remove)
    if pancard_number != "":
        pancard_list["PAN_no"] = pancard_number
    ...
    if date_of_birth == None:
        date_of_birth = extract_date_of_birth(english_string)
        pancard_list["DOB"] = date_of_birth

    fathers_name = get_father_name(simple_text_blank_remove)
    if fathers_name != "":
        pancard_list["Father_name"] = fathers_name
